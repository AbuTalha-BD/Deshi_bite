...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [currentUser, setCurrentUser] = useState<User | null>(() => {
    const saved = localStorage.getItem('deshi_bite_user');
    return saved ? JSON.parse(saved) : INITIAL_USERS[0]; // Default to Admin for instant preview demo
  });
    ...
    try {
      ...
      if (found) {
        if (found.status === 'PENDING') {
          showToast('Account registration is pending Admin approval.', 'error');
          setLoading(false);
          return false;
        }
        setCurrentUser(found);
        localStorage.setItem('deshi_bite_user', JSON.stringify(found));
